...
from ..models.right_apply import PermApply
...
class apply_become_provider(View):
    def post(self, request: HttpRequest, **kwargs) -> JsonResponse:
        ...
        PermApply.objects.create(group=group,
                                status=0,
                                applicant_id=applicant,
                                apply_time=time.time(),
                                reason=reason)
        return create_success_json_res_with({'apply_id':PermApply.objects.all()[PermApply.objects.count()-1].apply_id})
    
    def get(self, request: HttpRequest, device_id, **kwargs) -> JsonResponse:
        user = request.user
        applications = PermApply.objects.filter(applicant_id=user)
...
def post_apply_become_provider_apply_id_accept(request: HttpRequest, apply_id, **kwargs) -> JsonResponse:
    application = PermApply.objects.get(apply_id=apply_id)
